...
export default function AdminPortal({
  ...
  const [activeAdminTab, setActiveAdminTab] = useState<'overview' | 'pets' | 'applications' | 'users' | 'chats' | 'feedback' | 'audit'>('overview');
  ...
  const handleToggleShelfPet = (petId: string, currentStatus: string) => {
  ...
  const handleUpdateAppStatus = (appId: string, nextStatus: AdoptionApplication['status'], extraDetails?: string) => {
    const app = applications.find(a => a.id === appId);
    if (app) {
      if (nextStatus === '已通过') {
        pushAuditLog('核批领养成功', '领养申请审批', appId, `核准通过申请人 【${app.applicantNickname}】 对宠物 【${app.petName}】 的领养申请，匹配进入电子签约和回访生命周期。`);
        
        // Sync the actual pet status too
        setPets(prevPets => prevPets.map(p => p.id === app.petId ? { ...p, status: '已被领养', adopterId: app.applicantId } : p));
        
        // Generate feedback plan automatically for compliance check
        const planExists = feedbackPlans.some(f => f.petId === app.petId);
        if (!planExists) {
          const newPlan: FeedbackPlan = {
            id: `f_plan_${Date.now()}_${Math.random().toString(36).substr(2, 5)}`,
            petId: app.petId,
            petName: app.petName,
            petPhoto: app.petPhoto,
            adopterId: app.applicantId,
            adopterNickname: app.applicantNickname,
            ownerId: 'u_me', // simulated owner
            ownerNickname: '大白铲屎官',
            nodes: [
              { nodeId: 'day7', title: '领养第7天安全汇报', dueDate: '2026-06-01', status: '待提交' },
              { nodeId: 'day30', title: '领养第30天健康打卡', dueDate: '2026-06-25', status: '待提交' },
              { nodeId: 'day90', title: '领养第90天回访考察', dueDate: '2026-08-25', status: '待提交' },
              { nodeId: 'day180', title: '领养第180天终期打卡', dueDate: '2026-11-25', status: '待提交' }
            ]
          };
          setFeedbackPlans(prevPlans => [newPlan, ...prevPlans]);
        }
      } else if (nextStatus === '未通过') {
        pushAuditLog('驳回领养申请', '领养申请审批', appId, `驳回 【${app.applicantNickname}】 的申请意愿。原因批示：${extraDetails || '住房协议未达成/经验背景不符'}`);
      } else {
        pushAuditLog('推进审批步骤', '领养申请审批', appId, `将应征表进度更改为：【${nextStatus}】`);
      }
    }

    setApplications(prev => prev.map(a => {
      if (a.id === appId) {
        return { ...a, status: nextStatus, rejectReason: extraDetails };
      }
      return a;
    }));
  };
  ...
  const handleUserCreditAdjust = (e: React.FormEvent) => {
    e.preventDefault();
    ...
    setUsers(prev => prev.map(u => {
      if (u.id === creditAdjustmentUserId) {
        return u; // Under standard configuration this represents virtual metadata logging
      }
      return u;
    }));
  ...
  const handleReviewFeedbackNode = (planId: string, nodeId: string, verdict: '正常' | '有异常', adminRemarks: string) => {
    ...
    setFeedbackPlans(prev => prev.map(planItem => {
      if (planItem.id === planId) {
        return {
          ...planItem,
          nodes: planItem.nodes.map(n => {
            if (n.nodeId === nodeId) {
              return {
                ...n,
                status: '已提交', // Force mark verified state
  ...
  const stats = {
    ...
    activePets: pets.filter(p => p.status === '开放申请').length,
    adoptedPets: pets.filter(p => p.status === '已被领养').length,
    
    totalApps: applications.length,
    pendingApps: applications.filter(a => a.status === '已提交' || a.status === '沟通中').length,
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-sm font-extrabold text-white tracking-widest uppercase">流浪猫狗生老病死全周期治理系统</h1>
          ...
          <div className="mt-auto border-t border-slate-800/80 pt-3">
            <button 
              ...
              className={`w-full flex items-center gap-2 text-left px-3 py-2 rounded-xl transition-all ${
                activeAdminTab === 'audit' ? 'bg-slate-800 text-white font-semibold' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'
              }`}
            >
              <History className="w-4 h-4 shrink-0 text-slate-500" />
              <span className="text-xs">🪵 系统审计日志</span>
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 bg-slate-900/40 p-4 overflow-y-auto flex flex-col gap-4">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                <div className="bg-slate-950 p-3 rounded-2xl border border-slate-800 hover:border-indigo-500/30 transition-all group">
                  <div className="flex justify-between items-start">
                    <span className="text-[10px] font-bold text-slate-400 uppercase">宠物档案总量</span>
                    <span className="p-1 bg-indigo-500/10 text-indigo-400 rounded-lg text-xs">🐾</span>
                  </div>
                  <div className="mt-2 flex items-baseline gap-2">
                    <span className="text-2xl font-black text-white">{stats.totalPets}</span>
                    <span className="text-[9px] text-slate-450">在刊领养库</span>
                  </div>
                  <div className="mt-1.5 flex gap-2 text-[9px] border-t border-slate-800 pt-1.5 mt-2">
                    <span className="text-rose-400">● {stats.reviewingPets} 待审批审核</span>
                    <span className="text-emerald-400">● {stats.activePets} 开放领养中</span>
                  </div>
                </div>

                <div className="bg-slate-950 p-3 rounded-2xl border border-slate-800 hover:border-amber-500/30 transition-all">
                  <div className="flex justify-between items-start">
                    <span className="text-[10px] font-bold text-slate-400 uppercase">领养申请应征表</span>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 lg:col-span-2 space-y-3">
                  <span className="text-xs font-bold text-white block">📊 本市流浪动物生命周期流转趋势 (待领养 ➔ 签署协议 ➔ 打卡反馈)</span>
                  ...
                  <div className="h-44 flex items-end justify-between px-6 pt-4 relative">
                    ...
                    <div className="flex flex-col items-center gap-2 group z-10 w-16">
                      <div className="w-9 bg-rose-500 rounded-t-lg transition-all hover:bg-rose-450" style={{ height: `${(stats.activePets / 10) * 120 + 20}px` }}></div>
                      <span className="text-[9px] text-slate-400">待领养 ({stats.activePets})</span>
                    </div>

                    {/* Bar 2 */}
                    <div className="flex flex-col items-center gap-2 group z-10 w-16">
                      <div className="w-9 bg-pink-500 rounded-t-lg transition-all hover:bg-pink-450" style={{ height: `${(stats.reviewingPets / 10) * 120 + 10}px` }}></div>
                      <span className="text-[9px] text-slate-400">发布审核中 ({stats.reviewingPets})</span>
                    </div>

                    {/* Bar 3 */}
                    <div className="flex flex-col items-center gap-2 group z-10 w-16">
                      <div className="w-9 bg-amber-500 rounded-t-lg transition-all hover:bg-amber-450" style={{ height: `${(stats.pendingApps / 10) * 120 + 20}px` }}></div>
                      <span className="text-[9px] text-slate-400">应征审核中 ({stats.pendingApps})</span>
                    </div>

                    {/* Bar 4 */}
                    <div className="flex flex-col items-center gap-2 group z-10 w-16">
                      <div className="w-9 bg-emerald-500 rounded-t-lg transition-all hover:bg-emerald-450" style={{ height: `${(stats.adoptedPets / 10) * 120 + 35}px` }}></div>
                      <span className="text-[9px] text-slate-400">喜获新家 ({stats.adoptedPets})</span>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-900/60 p-3 rounded-xl border border-slate-850 flex items-center justify-between text-[11px] text-slate-400">
                    <div className="flex gap-2 items-center">
                      ...
                      <span>当前双向匹配爱心达成度高，健康防退养流程闭环完整。</span>
                    </div>
                    <button 
                      onClick={() => {
                        const targetMe = users.find(u => u.id === 'u_me') || users[0];
                        setUsers([...users]); // Virtual trigger
                            ...
                            <div className="flex items-center gap-2.5">
                              ...
                              <div>
                                ...
                                <span className="text-[9px] text-indigo-400 mt-0.5 block flex items-center gap-1">
                                  应聘对象: <strong className="text-white bg-indigo-500/10 px-1 rounded">{app.petName}</strong>
                                </span>
                              </div>
                            </div>
                          </td>
                          ...
                          <td className="p-3 text-right">
                            <div className="flex justify-end gap-1.5">
                              ...
                              <button 
                                ...
                                className="px-2 py-1 bg-emerald-655 hover:bg-emerald-600 text-white rounded text-[10px] font-extrabold cursor-pointer"
                                disabled={app.status === '已通过'}
                              >
                                准予同意
                              </button>
                    ...
                    <div className="space-y-3.5 text-[11px] self-start bg-slate-900/50 p-3 rounded-2xl border border-slate-850">
                      ...
                      <div className="flex gap-2 pt-2">
                        <button 
                          ...
                          className="flex-1 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded text-center font-bold text-xs cursor-pointer"
                        >
                          核准批准
                        </button>
                        <button 
          ...
          {activeAdminTab === 'users' && (
            <div className="space-y-4">
                      ...
                      {filteredUsers.map(user => (
                        <tr key={user.id} className="hover:bg-slate-900/30 transition-colors">
                          ...
                          <td className="p-3">
                            <div className="space-y-1">
                              <div className="flex items-center gap-1">
                                ...
                                <span className="text-xs font-black text-emerald-450 font-mono">98/100</span>
                                <span className="bg-emerald-500/10 text-emerald-400 text-[8px] font-extrabold px-1 rounded scale-90">优异</span>
                              </div>
                          ...
                          <td className="p-3 text-right">
                            <div className="flex justify-end gap-1.5">
                              <button 
                                onClick={() => setCreditAdjustmentUserId(user.id)}
              ...
              {creditAdjustmentUserId && (
                <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-3">
                  <div className="flex justify-between items-center border-b border-slate-850 pb-1.5">
                    <span className="text-xs font-extrabold text-white">✍ 调节实名勋章和信用评分</span>
                  ...
                  <form onSubmit={handleUserCreditAdjust} className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                    ...
                    <div>
                      ...
                      <input 
                        ...
                        onChange={(e) => setCreditAdjustmentPoints(parseInt(e.target.value))}
                        className="w-full bg-slate-900 border border-slate-855 rounded px-2.5 py-1 text-slate-200"
                        required
                      />
                    </div>
                        ...
                        {plan.nodes.map(n => {
                          ...
                          return (
                            <div key={n.nodeId} className="p-1 rounded bg-slate-950 text-center border border-slate-850">
                              <span className="text-[8px] text-slate-500 block leading-none">{n.title.slice(3, 7)}</span>
                ...
                <div className="bg-slate-950 p-3.5 rounded-2xl border border-slate-800 flex flex-col justify-between">
                  <div>
                    ...
                    <div className="bg-slate-900 p-3 rounded-xl border border-slate-850 space-y-2.5 text-xs text-slate-300">
                      <div className="flex justify-between text-[10px]">
                        <span className="text-slate-450">打卡自述:</span>
                        <span className="text-emerald-400 font-mono">2026-05-24 天河</span>
                      </div>
                      <p className="leading-relaxed bg-slate-950 p-2 rounded border border-slate-850 text-slate-200">
                        “奶茶来到新家已经一周啦！已经非常适应，每天吃好喝好，开始在猫砂盆里熟练排泄，还会跟家里的大狗友好闻鼻子，一点也不欺负。吃的是皇家全价橘猫粮，感谢原发布者的热心指导！”
                      </p>

                      <div className="grid grid-cols-3 gap-2 py-0.5 text-[10px]">
                        <div>
                          <span className="text-slate-500 text-[8px] block">精神食欲评分</span>
                          <strong className="text-emerald-450">很好</strong>
                        </div>
                        <div>
                          <span className="text-slate-500 text-[8px] block">排便猫砂状态</span>
                          <strong className="text-emerald-455">100% 规律</strong>
                        </div>
                        <div>
                          <span className="text-slate-500 text-[8px] block">家庭适应状态</span>
                          <strong className="text-indigo-405">完全融入新家</strong>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="border-t border-slate-850 pt-2.5 mt-3 space-y-2 text-xs">
                    ...
                    <div className="flex gap-1.5">
                      <button 
                        onClick={() => handleReviewFeedbackNode(feedbackPlans[0]?.id || 'plan1', 'day7', '正常', '情况评估：一切优秀，无任何不良反馈或违纪行为，领养安全。')}
                        className="flex-1 py-1 px-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded font-bold cursor-pointer text-center text-[10px]"
                      >
                        ✅ 无异常归档
                      </button>
                      <button 
                        onClick={() => {
                          ...
                          if (note) handleReviewFeedbackNode(feedbackPlans[0]?.id || 'plan1', 'day7', '有异常', note);
                        }}
                        className="flex-1 py-1 px-2 bg-rose-700/80 hover:bg-rose-600 text-rose-100 rounded font-bold cursor-pointer text-center text-[10px]"
                      >
                        ⚠️ 下发异常干预整改
                      </button>
                    </div>
