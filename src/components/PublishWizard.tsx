...
interface PublishWizardProps {
  onPublishComplete: (newListing: PetListing) => void;
  onCancel: () => void;
  publisherId: string;
...
export default function PublishWizard({ onPublishComplete, onCancel, publisherId, publisherNickname }: PublishWizardProps) {
  ...
  // Step 2: Base Info
  const [name, setName] = useState('蛋挞');
  const [breed, setBreed] = useState('中华田园橘猫');
  const [age, setAge] = useState('6个月');
  const [gender, setGender] = useState<'妹妹' | '弟弟'>('妹妹');
  const [weight, setWeight] = useState('2.8kg');
  ...
  const [healthPhotos, setHealthPhotos] = useState<string[]>([
    'https://images.unsplash.com/photo-1576091160550-2173dba999ef?auto=format&fit=crop&q=80&w=200' // mock medical certificate
  ]);
  ...
  const [desc, setDesc] = useState('蛋挞是一只超级粘人听话的小橘猫，浑身肉呼呼的。会用各种猫砂，不挑食，超级喜欢吃小冻干。刚带回家可能需要一两天适应，只要给零食立刻就能踩奶摸摸，是个百分百神仙猫咪，绝不哈人咬人！');

  // Step 5: Reasoning & conditions
  const [reason, setReason] = useState('由于房东临时通知不给继续喂养，并且我下个月要进行外派实习，合租室友有些过敏严重。希望帮它寻找一生相伴、绝不在任何情况下弃养的好爸爸妈妈。');
  const [requirements, setRequirements] = useState('1. 广州同城领养，全屋高网金刚砂窗防坠落。\n2. 喂食中端以上猫粮，进口主食罐头佳，科学精细喂养。\n3. 同意签署平台领求协议，配合后期7天、30天、180天节点及时提交返图。');
  ...
  // Since we are in simulation, we use beautifully prefilled image URLs which the user can clear or use.
  const [petPhotos, setPetPhotos] = useState<string[]>([
    'https://images.unsplash.com/photo-1543466835-00a7907e9de1?auto=format&fit=crop&q=80&w=500', // Photo 1
    'https://images.unsplash.com/photo-1514888286974-6c03e2ca1dba?auto=format&fit=crop&q=80&w=500', // Photo 2
    'https://images.unsplash.com/photo-1533738363-b7f9aef128ce?auto=format&fit=crop&q=80&w=500'  // Photo 3
  ]);
  ...
  const handlePublishSubmit = () => {
    setStep(8); // Enter Pending Audit state
  };

  const handleConfirmAuditSuccess = () => {
    // Simulated approval action triggering complete dispatch up
    const newListing: PetListing = {
      ...
      status: '开放申请', // Directly active after audit bypass click!
      createdAt: new Date().toISOString().split('T')[0],
      views: 1,
      favorites: 0
    };
    onPublishComplete(newListing);
  };

  return (
    <div className="flex flex-col flex-1 h-full bg-slate-50 relative select-none" id="publish-wizard-root">
      ...
      {/* Header */}
      <div className="bg-white px-4 py-3 border-b border-slate-200 flex items-center justify-between shrink-0 sticky top-0 z-30 shadow-sm">
        <button onClick={onCancel} className="text-slate-500 font-medium text-xs py-1 px-2 rounded hover:bg-slate-100">
          退出发布
        </button>
        <span className="font-bold text-xs text-slate-800">
          {step <= 7 ? `发布领养 (${step}/7)` : '线上流浪动物审核'}
        </span>
      ...
      {step <= 7 && (
        <div className="bg-white px-4 py-2 border-b border-slate-100 flex justify-between text-[10px] font-bold text-slate-400 gap-1 shrink-0 overflow-x-auto">
          <span className={step === 1 ? 'text-rose-500' : 'text-slate-300'}>1.类型</span>
          <ChevronRight className="w-3 h-3 text-slate-200 shrink-0" />
          <span className={step === 2 ? 'text-rose-500' : 'text-slate-300'}>2.基本</span>
          <ChevronRight className="w-3 h-3 text-slate-200 shrink-0" />
          <span className={step === 3 ? 'text-rose-500' : 'text-slate-300'}>3.健康</span>
          <ChevronRight className="w-3 h-3 text-slate-200 shrink-0" />
          <span className={step === 4 ? 'text-rose-500' : 'text-slate-300'}>4.习性</span>
          <ChevronRight className="w-3 h-3 text-slate-200 shrink-0" />
          <span className={step === 5 ? 'text-rose-500' : 'text-slate-300'}>5.要求</span>
          <ChevronRight className="w-3 h-3 text-slate-200 shrink-0" />
          <span className={step === 6 ? 'text-rose-500' : 'text-slate-300'}>6.配图</span>
          <ChevronRight className="w-3 h-3 text-slate-200 shrink-0" />
          <span className={step === 7 ? 'text-rose-500' : 'text-slate-300'}>7.预览</span>
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
        ...
        {step === 8 && (
          <div className="flex flex-col gap-5 items-center justify-center my-auto text-center px-4">
            ...
            {/* Simulate administrator check bypass block */}
            <div className="border border-indigo-200 rounded-lg bg-indigo-50 p-3 w-full text-center mt-4">
              <span className="text-[10px] font-bold text-indigo-800 block mb-2">💡 管理员控制面板 (演示开发专用)</span>
              <button
                type="button"
                onClick={handleConfirmAuditSuccess}
                className="w-full bg-indigo-600 text-white font-bold py-2 text-xs rounded-lg active:scale-95 transition-transform flex items-center justify-center gap-1 hover:bg-indigo-700"
              >
                <Check className="w-4 h-4" />
                <span>模拟平台管理员审核：点击一键通过并上线</span>
              </button>
            </div>
          </div>
        )}
        ...
        <div className="bg-white px-4 py-3 border-t border-slate-200 flex gap-2 shrink-0">
          ...
          {step < 7 ? (
            <button
              onClick={handleNext}
              type="button"
              className="flex-[2] py-2 text-xs font-bold text-white bg-rose-500 hover:bg-rose-600 rounded-lg flex items-center justify-center gap-1 active:scale-95 transition-all"
            >
              <span>保存并下一步</span>
            ...
            <button
              ...
              type="button"
              className="flex-[2] py-2 text-xs font-black text-white bg-emerald-500 hover:bg-emerald-600 rounded-lg flex items-center justify-center gap-1 active:scale-95 transition-all"
            >
